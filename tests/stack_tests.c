#include <stdbool.h> // bool, true, false
#include <stdio.h>   // printf

#define VALUE_TYPE int
#define VALUE_NAME int
#include "../src/stack.h"

bool empty_test(void) {
    stack_int_t* s;
    if (!stack_int_init(&s, 1)) {
        return false;
    }
    bool res = true;

    res &= stack_int_isempty(s);
    res &= stack_int_deinit(&s);

    return res;
}

bool one_element_test(void) {
    stack_int_t* s;
    if (!stack_int_init(&s, 1)) {
        return false;
    }
    bool res = true;
    int value = 5;
    stack_int_push(s, value);

    res &= value == stack_int_peek(s);
    res &= !stack_int_isempty(s);

    res &= value == stack_int_pop(s);
    res &= stack_int_isempty(s);
    res &= !stack_int_isfull(s);

    res &= stack_int_deinit(&s);

    return res;
}

bool million_elements_test(void) {
    stack_int_t* s;
    if (!stack_int_init(&s, 1000000)) {
        return false;
    }
    bool res = true;
    for (int i = 1; i <= 1000000; i++) {
        stack_int_push(s, i);
    }
    for (int i = 1000000; i >= 1; i--) {
        res &= i == stack_int_pop(s);
    }
    res &= stack_int_deinit(&s);
    return res;
}

bool grow_test(void) {
    stack_int_t* s;
    if (!stack_int_init(&s, 500000)) {
        return false;
    }
    bool res = true;
    for (int i = 1; i <= 500000; i++) {
        stack_int_push(s, i);
    }

    res &= stack_int_isfull(s);
    if (!stack_int_resize(s, s->used << 1)) {
        stack_int_deinit(&s);
        return false;
    }
    res &= !stack_int_isfull(s);

    for (int i = 500001; i <= 1000000; i++) {
        stack_int_push(s, i);
    }
    for (int i = 1000000; i >= 1; i--) {
        res &= i == stack_int_pop(s);
    }
    res &= stack_int_deinit(&s);
    return res;
}

bool shrink_test(void) {
    stack_int_t* s;
    if (!stack_int_init(&s, 500000)) {
        return false;
    }
    bool res = true;
    for (int i = 1; i <= 250000; i++) {
        stack_int_push(s, i);
    }

    res &= !stack_int_isfull(s);
    if (!stack_int_resize(s, s->used)) {
        stack_int_deinit(&s);
        return false;
    }
    res &= stack_int_isfull(s);

    for (int i = 250000; i >= 1; i--) {
        res &= i == stack_int_pop(s);
    }
    res &= stack_int_deinit(&s);
    return res;
}

bool foreach_test(void) {
    stack_int_t* s;
    if (!stack_int_init(&s, 50)) {
        return false;
    }
    bool res = true;
    for (int i = 51; i <= 100; i++) {
        stack_int_push(s, i);
    }
    int value = 51;
    int var;
    size_t i;
    stack_foreach(s, var, i) {
        res &= value == var;
        value++;
    }
    return res;
}

typedef bool (*bool_f)(void);

typedef struct {
    bool_f func;
    const char* desc;
} func_plus;

int main(void) {
    func_plus bool_f_arr[] = {
        {empty_test, "empty test"}, {one_element_test, "one element test"}, {million_elements_test, "million elements test"},
        {grow_test, "grow test"},   {shrink_test, "shrink test"},           {foreach_test, "foreach test"}};
    for (size_t i = 0; i < sizeof(bool_f_arr) / sizeof(func_plus); i++) {
        printf("[%s] %s\n", bool_f_arr[i].func() ? "true" : "false", bool_f_arr[i].desc);
    }
    return 0;
}
