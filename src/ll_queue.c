...
ll_queue_node_type* ll_queue_node_create(size_t value_size) {
    ...
    ll_queue_node_p->value_p = malloc(value_size);
...
bool ll_queue_copy(ll_queue_type** ll_queue_dest_pp, const ll_queue_type* ll_queue_src_p) {
    ...
    if (!ll_queue_init(ll_queue_dest_pp)) {
        return false;
    }

    ll_queue_node_type* head_p = ll_queue_src_p->head_p;
    while (head_p != NULL) {
        ll_queue_node_type* head_copy_p = ll_queue_node_create(head_p->value_size);
        if (head_copy_p == NULL) {
