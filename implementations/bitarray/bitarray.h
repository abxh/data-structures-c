...
Bitarray* bitarray_new(size_t num_of_bits); /* Create a new bitarray of a given size with values initalized to 0. Return value is NULL if OOM or num_of_bits is 0. */
...
bool bitarray_get_safe(const Bitarray* bitarray_p, size_t index, char* filename, int linenr); /* Return the value at an index. With OOB check. */
void bitarray_set_true_safe(Bitarray* bitarray_p, size_t index, char* filename, int linenr); /* Set the value to true at an index. With OOB check. */
void bitarray_set_false_safe(Bitarray* bitarray_p, size_t index, char* filename, int linenr); /* Set the value to false at an index. With OOB check. */
void bitarray_set_safe(Bitarray* bitarray_p, size_t index, bool bit, char* filename, int linenr); /* Set a provided value at an index. With OOB check. */
void bitarray_toggle_safe(Bitarray* bitarray_p, size_t index, char* filename, int linenr); /* Toggle the value at an index. With OOB check. */
