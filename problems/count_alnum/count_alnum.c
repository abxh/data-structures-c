...
int main(void) {
    bitarray_type* bitarray_p = bitarray_create_with_min_num_of_bits(TOTAL);
    ...
    for (int c = fgetc(stdin); c != EOF; c = fgetc(stdin)) {
        if (islower(c)) {
            bitarray_set_true_at(bitarray_p, c - 'a');
        } else if (isupper(c)) {
            bitarray_set_true_at(bitarray_p, c - 'A' + NUM_OF_LETTERS);
        } else if (isdigit(c)) {
            bitarray_set_true_at(bitarray_p, c - '0' + 2 * NUM_OF_LETTERS);
        }
    }
